...
/**
 * アカウントトークンとQUESTの場所
 * @typePartId 1090bf9819b59140978ca12f5f1bf18e
 */
export type AccountTokenAndQLocation = { 
/**
 * アカウントトークン
 */
...
/**
 * quest で 生徒が取得する クラスの質問の木構造と, 自分の回答と確定状態
 * @typePartId 57936f63c32ff98bf54918c8dd0772d6
 */
...
/**
 * 自分の答えた回答と確定状態. まだ答えていない場合は, Noting
 */
readonly answer: Maybe<StudentAnswerAndIsConfirm> };
 ...
 * 型のパラメータ
 */
readonly parameter: List<DataTypeOrDataTypeParameter> };
 ...
 * @typePartId 8853d36cdb4a306f289bcd09a0fb87d9
 */
export type QUrlData = { readonly _: "Normal"; readonly qLocation: QLocation } | { readonly _: "LogInCallback"; readonly codeAndState: CodeAndState };
...
/**
 * Definy の 関数を含んだ型. 式や値に対して決まる. 型パーツの定義にも使う
 * @typePartId b3b36f39469d23321ed01b92f048ccc0
 */
export type Type = { 
/**
 * 関数の入力.
 * Notiong の場合は単なる値だということ
 */
 ...
 * @typePartId c2341cd0bfc0050f83bc12c230d3c89c
 */
...
/**
 * quest で自分自身が答えた回答と確定状態
 * @typePartId fc42881441e48fb158de75fdbc082025
 */
...
readonly helper: (a: QClassStudentOrGuest) => QClassStudentOrGuest } = { typePartId: "067954ebc8a7f155be3a4ee196ccf99a" as TypePartId, helper: (qClassStudentOrGuest: QClassStudentOrGuest): QClassStudentOrGuest => qClassStudentOrGuest, codec: { encode: (value: QClassStudentOrGuest): ReadonlyArray<number> => (String.codec.encode(value.name).concat(QClassId.codec.encode(value.id)).concat(AccountId.codec.encode(value.createAccountId)).concat(QProgramId.codec.encode(value.programId))), decode: (index: number, binary: Uint8Array): { readonly result: QClassStudentOrGuest; readonly nextIndex: number } => {
  ...
  const programIdAndNextIndex: { readonly result: QProgramId; readonly nextIndex: number } = QProgramId.codec.decode(createAccountIdAndNextIndex.nextIndex, binary);
  return { result: { name: nameAndNextIndex.result, id: idAndNextIndex.result, createAccountId: createAccountIdAndNextIndex.result, programId: programIdAndNextIndex.result }, nextIndex: programIdAndNextIndex.nextIndex };
} } };
 ...
 * @typePartId 1090bf9819b59140978ca12f5f1bf18e
 */
export const AccountTokenAndQLocation: { 
/**
 * definy.app内 の 型パーツの Id
 */
readonly typePartId: TypePartId; 
 ...
 * 独自のバイナリ形式の変換処理ができるコーデック
 */
readonly codec: Codec<AccountTokenAndQLocation>; 
/**
 * 型を合わせる上で便利なヘルパー関数
 */
readonly helper: (a: AccountTokenAndQLocation) => AccountTokenAndQLocation } = { typePartId: "1090bf9819b59140978ca12f5f1bf18e" as TypePartId, helper: (accountTokenAndQLocation: AccountTokenAndQLocation): AccountTokenAndQLocation => accountTokenAndQLocation, codec: { encode: (value: AccountTokenAndQLocation): ReadonlyArray<number> => (AccountToken.codec.encode(value.accountToken).concat(QLocation.codec.encode(value.qLocation))), decode: (index: number, binary: Uint8Array): { readonly result: AccountTokenAndQLocation; readonly nextIndex: number } => {
  ...
  const qLocationAndNextIndex: { readonly result: QLocation; readonly nextIndex: number } = QLocation.codec.decode(accountTokenAndNextIndex.nextIndex, binary);
  return { result: { accountToken: accountTokenAndNextIndex.result, qLocation: qLocationAndNextIndex.result }, nextIndex: qLocationAndNextIndex.nextIndex };
} } };
 ...
 * 型を合わせる上で便利なヘルパー関数
 */
readonly helper: (a: StudentSelfQuestionTree) => StudentSelfQuestionTree } = { typePartId: "57936f63c32ff98bf54918c8dd0772d6" as TypePartId, helper: (studentSelfQuestionTree: StudentSelfQuestionTree): StudentSelfQuestionTree => studentSelfQuestionTree, codec: { encode: (value: StudentSelfQuestionTree): ReadonlyArray<number> => (QQuestionId.codec.encode(value.questionId).concat(String.codec.encode(value.questionText)).concat(List.codec(StudentSelfQuestionTree.codec).encode(value.children)).concat(Maybe.codec(StudentAnswerAndIsConfirm.codec).encode(value.answer))), decode: (index: number, binary: Uint8Array): { readonly result: StudentSelfQuestionTree; readonly nextIndex: number } => {
  const questionIdAndNextIndex: { readonly result: QQuestionId; readonly nextIndex: number } = QQuestionId.codec.decode(index, binary);
  ...
  const answerAndNextIndex: { readonly result: Maybe<StudentAnswerAndIsConfirm>; readonly nextIndex: number } = Maybe.codec(StudentAnswerAndIsConfirm.codec).decode(childrenAndNextIndex.nextIndex, binary);
...
readonly helper: (a: DataType) => DataType } = { typePartId: "7e689d3cd011d42947cca759ecebd1cf" as TypePartId, helper: (dataType: DataType): DataType => dataType, codec: { encode: (value: DataType): ReadonlyArray<number> => (TypePartId.codec.encode(value.typePartId).concat(List.codec(DataTypeOrDataTypeParameter.codec).encode(value.parameter))), decode: (index: number, binary: Uint8Array): { readonly result: DataType; readonly nextIndex: number } => {
  ...
  const parameterAndNextIndex: { readonly result: List<DataTypeOrDataTypeParameter>; readonly nextIndex: number } = List.codec(DataTypeOrDataTypeParameter.codec).decode(typePartIdAndNextIndex.nextIndex, binary);
 ...
 * 独自のバイナリ形式の変換処理ができるコーデック
 */
readonly codec: Codec<QUrlData>; 
/**
 * 普通のページ
 */
readonly Normal: (a: QLocation) => QUrlData; 
/**
 * ソーシャルログインしたときに返ってくるURL
 */
readonly LogInCallback: (a: CodeAndState) => QUrlData } = { Normal: (qLocation: QLocation): QUrlData => ({ _: "Normal", qLocation }), LogInCallback: (codeAndState: CodeAndState): QUrlData => ({ _: "LogInCallback", codeAndState }), typePartId: "8853d36cdb4a306f289bcd09a0fb87d9" as TypePartId, codec: { encode: (value: QUrlData): ReadonlyArray<number> => {
  switch (value._) {
    case "Normal": {
      return [0].concat(QLocation.codec.encode(value.qLocation));
    }
    case "LogInCallback": {
      return [1].concat(CodeAndState.codec.encode(value.codeAndState));
    }
  }
}, decode: (index: number, binary: Uint8Array): { readonly result: QUrlData; readonly nextIndex: number } => {
  const patternIndex: { readonly result: number; readonly nextIndex: number } = Int32.codec.decode(index, binary);
  if (patternIndex.result === 0) {
    const result: { readonly result: QLocation; readonly nextIndex: number } = QLocation.codec.decode(patternIndex.nextIndex, binary);
    return { result: QUrlData.Normal(result.result), nextIndex: result.nextIndex };
  }
  if (patternIndex.result === 1) {
    ...
    return { result: QUrlData.LogInCallback(result.result), nextIndex: result.nextIndex };
  }
  throw new Error("存在しないパターンを指定された 型を更新してください");
} } };
...
readonly helper: (a: Project) => Project } = { typePartId: "8bc81fa2a15bdbd8c2414d22f3e1b462" as TypePartId, helper: (project: Project): Project => project, codec: { encode: (value: Project): ReadonlyArray<number> => (String.codec.encode(value.name).concat(ImageHash.codec.encode(value.iconHash)).concat(ImageHash.codec.encode(value.imageHash)).concat(Time.codec.encode(value.createTime)).concat(AccountId.codec.encode(value.createAccountId)).concat(Time.codec.encode(value.updateTime)).concat(ProjectId.codec.encode(value.id))), decode: (index: number, binary: Uint8Array): { readonly result: Project; readonly nextIndex: number } => {
  ...
  const idAndNextIndex: { readonly result: ProjectId; readonly nextIndex: number } = ProjectId.codec.decode(updateTimeAndNextIndex.nextIndex, binary);
  return { result: { name: nameAndNextIndex.result, iconHash: iconHashAndNextIndex.result, imageHash: imageHashAndNextIndex.result, createTime: createTimeAndNextIndex.result, createAccountId: createAccountIdAndNextIndex.result, updateTime: updateTimeAndNextIndex.result, id: idAndNextIndex.result }, nextIndex: idAndNextIndex.nextIndex };
} } };
 ...
 * 型を合わせる上で便利なヘルパー関数
 */
readonly helper: (a: StudentAnswerAndIsConfirm) => StudentAnswerAndIsConfirm } = { typePartId: "fc42881441e48fb158de75fdbc082025" as TypePartId, helper: (studentAnswerAndIsConfirm: StudentAnswerAndIsConfirm): StudentAnswerAndIsConfirm => studentAnswerAndIsConfirm, codec: { encode: (value: StudentAnswerAndIsConfirm): ReadonlyArray<number> => (String.codec.encode(value.text).concat(Bool.codec.encode(value.isConfirm))), decode: (index: number, binary: Uint8Array): { readonly result: StudentAnswerAndIsConfirm; readonly nextIndex: number } => {
